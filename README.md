...
| **Unlit panel** | Shows dimly visible unlit LED dots in the background, simulating a real LED matrix panel. Off by default. |
| **Canvas background** | Sets the background color behind the tree: Dark grey (default), Black, White, or Old parchment. |
...
Six tree types are implemented. Five are currently selectable; **Literati is disabled** in this build (it looks too sparse in the LED-dot medium). Its code is retained, commented out, and can be re-enabled later.
...
| **Literati** *(disabled)* | Inspired by the formal Japanese *bunjin* (literati) bonsai style. A thin trunk meanders dramatically with each layer. Branching is suppressed in the lower half of the tree and sparse near the apex, concentrating foliage into a few expressive clusters at the top. Looks painterly and minimal compared to the denser types. |
...
All currently-enabled types are included in the **🎰 Randomize All** rotation.
...
- **In focus mode**, five buttons appear in the top-right corner (right to left):
  - **⊡** — exit focus mode
  - **⏸ / ▶** — pause/resume
...
The **⛶** button (leftmost in the focus mode toolbar) requests true OS-level fullscreen, hiding the browser's address bar and system UI. The button icon changes to indicate the current state and glows accent green while fullscreen is active.

- Exiting focus mode (via the exit button or **Escape**) also exits fullscreen automatically.
- If the user exits fullscreen via the device's system back gesture, the button syncs to reflect the new state.
- The button is hidden automatically on browsers that don't support the Fullscreen API (e.g. iOS Safari).
...
| **1.17** | Added fullscreen button to focus mode toolbar (Web Fullscreen API); auto-exits fullscreen when leaving focus mode; hidden on unsupported browsers; SVG icon toggles between enter/exit states |
| **1.16** | Replaced focus mode enter/exit button Unicode symbols (⛶/⊠) with inline SVG icons to fix colored rendering on Android Chrome |
| **1.15** | Replaced pause/play button Unicode symbols (⏸/▶) with inline SVG icons to fix colored rendering on Android Chrome; icon swap now handled by CSS class rather than JS textContent |
| **1.14** | Added `-webkit-appearance: none` and Unicode VS15 variation selectors to focus mode buttons to suppress Android Chrome emoji styling (partial fix, superseded by 1.15/1.16) |
| **1.13** | Fixed crash in `CascadeTree.drawLeavesAtTip` when using bare palettes (e.g. Winter Bare): added `palette.bare` early-exit guard, null-safety on `palette.leaves` and `palette.fruit.colors`, and a `null` color skip in the particle loop |
