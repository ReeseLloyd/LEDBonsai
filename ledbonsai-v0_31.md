...
**Seed-free bookmarks are supported.** `loadFromUrl()` activates if any known param is present — a `seed` is no longer required. This enables pure display-mode bookmarks with no specific tree:

```
ledbonsai.html?screensaver=1&ssinterval=60&focus=1
```

**Focus mode launch timing** — focus mode cannot activate until after the first tree is rendered (so `fitToViewport()` has real dimensions to work with). `loadFromUrl()` sets `this.pendingFocusMode = true` instead of activating immediately; `generate()` consumes this flag at the very end of its first run.
...
- **`randomizeAll()` clears the seed field** — `getOptions()` then generates a fresh random seed and writes it back to the input. Both the screensaver and 🎰 button call `randomizeAll()`, ensuring consistent behavior.
- **`startScreesaver()` typo** — single `e` in method name. Don't fix without updating all call sites.
- **Focus progress bar animation restart** — `void bar.offsetWidth` forces a reflow between removing and re-adding the `running` class, required for the CSS animation to restart from the beginning each cycle.
...
- **`pendingFocusMode` flag** — set by `loadFromUrl()` when `focus=1` is in the URL. Consumed (and cleared) at the end of the first `generate()` call. Prevents focus mode from activating before the tree exists in the DOM.
...
| v0.26 | Zoom step reduced to 10% (min 10%); focus mode added (`⛶`/`⊠` toggle, `Escape` to exit) |
...
- **More bark styles** — could add Pine, Oak furrows; RNG budget is pre-reserved so any palette can gain bark without shifting tree shapes
